...
async def run_simulation(input: SimulationInput):
    ...
    for _ in range(input.num_simulations):
        path_data = []
        current_value = input.initial_corpus
        
        # Expense logic
        expense = input.current_monthly_expense * 12  # Start with annual expense
        ...
        for year in range(1, simulation_horizon):
            ...
            # Generate random inflation based on normal distribution
            random_inflation = np.random.normal(inflation, inflation_std_dev)
            
            # Update portfolio value
            current_value *= (1 + random_return)
            
            # Adjust expense for inflation (Monte Carlo)
